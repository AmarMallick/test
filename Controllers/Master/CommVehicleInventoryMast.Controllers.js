...
exports.vehicle_inventory_mast_fill = async (req, res) => {
  ...
  await form(req, res, async (err) => {
    ...
    try {
      connection.query(
        "call sp_vehicle_inventory_mast_fill(?,?)",
...
exports.vehicle_inventory_mast_save = async (req, res) => {
  ...
  await cpUpload(req, res, async (err) => {
    ...
    try {
      if (in_v_title == "") {
      ...
      } else if (in_v_type_id == null) {
        res.status(400).json({ data: [{ message: "Select vehicle type " }] });
        ...
        connection.query(
          "call sp_vehicle_inventory_mast_save(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
          [
...
exports.vehicle_inventory_mast_delete = async (req, res) => {
  var form = multer().array([]);
  await form(req, res, async (err) => {
    let in_vehicle_inventory_id = req.body.vehicle_inventory_id;
    const userId = req.user;
    ...
    try {
      if (!in_vehicle_inventory_id) {
        res
          .status(400)
          .json({ data: [{ message: "Enter vehicle_inventory_id " }] });
      } else if (!in_user_id) {
        res.status(400).json({ data: [{ message: "Enter user_id " }] });
      } else {
        connection.query(
          "call sp_vehicle_inventory_mast_delete(?,?)",
