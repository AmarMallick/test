...
exports.commercial_brand_mast_fill = async (req, res) => {
  var form = multer().array([]);
  await form(req, res, async (err) => {
    try {
      connection.query("call sp_commercial_brand_mast_fill", (err, result) => {
        if (err) {
...
exports.commercial_brand_mast_save = async (req, res) => {
  var form = multer().array([]);
  await form(req, res, async (err) => {
    ...
    try {
      if (!in_id) {
        ...
        connection.query(
          "call sp_commercial_brand_mast_save(?,?)",
          [in_id, in_name],
          (err, result) => {
            if (err) {
              res.status(500).json({ success: 0, data: err });
...
exports.commercial_brand_mast_delete = async (req, res) => {
  var form = multer().array([]);
  await form(req, res, async (err) => {
    let in_id = req.body.id;
    ...
    try {
      if (!in_id) {
        ...
        connection.query(
          "call sp_commercial_brand_mast_delete(?)",
          [in_id],
          (err, result) => {
            if (err) {
              res.status(500).json({ success: 0, data: err });
